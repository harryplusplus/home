...
이미지 파일은 Monolith 서버와 Postgres를 사용해서 저장하고 읽을 수 도 있습니다.
하지만 이렇게 구현할 경우, egress 요금이 불필요하게 나올 수 있다고 생각합니다.
Monolith 서버에서 Relay를 해야하기 때문입니다.

반면에 AWS S3는 Presigned URL을 발급받을 수 있습니다.
Presigned URL은 일시적으로 읽기 전용 URL을 생성할 수 있습니다.
Presigned URL을 사용하면 서버에서 Relay를 할 필요가 없어집니다.
그리고 Supabase Storage는 AWS S3 호환 파일 저장소를 지원합니다.
